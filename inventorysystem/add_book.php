...
<div class="form-container glass-card">
  ...
  <!-- Duplicate warning -->
  <div class="dup-banner" id="dup_banner">
    ⚠ <strong id="dup_title"></strong> already exists in your inventory (qty: <strong id="dup_qty"></strong>).
    &nbsp;<a href="#" id="dup_link">Update stock instead?</a>
  </div>
...
async function checkDuplicate(title, isbn) {
  ...
  const data = await r.json();
  const banner = document.getElementById('dup_banner');
  if (data.duplicate) {
    document.getElementById('dup_title').textContent = data.book_title;
    document.getElementById('dup_qty').textContent   = data.quantity;
    document.getElementById('dup_link').href         = `view_books.php?edit=${data.book_id}`;
